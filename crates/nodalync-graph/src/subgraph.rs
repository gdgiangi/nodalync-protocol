use crate::{Entity, Relationship, EntitySource, SubgraphResult, GraphError};
use anyhow::Result;
...
pub fn get_subgraph(
    ...
    while let Some((entity_id, depth)) = queue.pop_front() {
        ...
        for relationship in entity_relationships {
            ...
            if !all_relationships.iter().any(|r: &Relationship| r.id == relationship.id) {
...
pub fn get_entities_by_type(conn: &Connection, entity_type: &str, limit: u32) -> Result<Vec<Entity>> {
    let query = "
        SELECT e.id, e.canonical_label, e.entity_type, e.description,
               e.confidence, e.first_seen, e.last_updated, e.source_count,
               e.metadata_json, GROUP_CONCAT(a.alias) as aliases
