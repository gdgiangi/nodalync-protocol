...
use nodalync_store::{ContentStore, ManifestStore, NodeState, NodeStateConfig};
...
fn create_sample_content(id: u64, size: usize) -> Vec<u8> {
    format!(
        "Content {} with {} bytes of data: {}",
        id,
        size,
        "X".repeat(size.saturating_sub(50))
    )
    .into_bytes()
}
...
fn bench_batch_content_operations(c: &mut Criterion) {
    ...
    for count in [10, 50, 100, 500].iter() {
                ...
                b.iter_with_setup(
                    || {
                        ...
                        let contents: Vec<_> = (0..count)
                            .map(|i| create_sample_content(i as u64, 512))
                            .collect();
        ...
        group.bench_with_input(
            BenchmarkId::new("retrieve_batch", count),
            count,
            |b, &count| {
                b.iter_with_setup(
                    || {
...
fn bench_announcement_search(c: &mut Criterion) {
    ...
    c.bench_function("search_announcements_filter", |b| {
        b.iter(|| {
            let results = state.search_announcements(
                black_box(""),
                Some(black_box(ContentType::L1)),
                black_box(100),
            );
            black_box(results);
        });
...
fn bench_database_scaling(c: &mut Criterion) {
    ...
    for count in [100, 1000, 5000, 10000].iter() {
        group.bench_with_input(
            BenchmarkId::new("announcement_count", count),
            count,
            |b, &count| {
                b.iter_with_setup(
                    || {
        ...
        group.bench_with_input(
            BenchmarkId::new("search_in_large_db", count),
            count,
            |b, &count| {
                b.iter_with_setup(
                    || {
                    ...
                    |(state, _temp_dir)| {
                        let results =
                            state.search_announcements(black_box("special"), None, black_box(10));
                        black_box(results);
                    },
                );
            },
